...
function popPredict(me) {
  ...
  const egg = eggPredict(0.9, me)
  if (egg) foresight.unlocks.push(egg);
  return foresight;
}
...
function eggPredict(failRate, shimmer) {
  failRate *= 1 / Game.dropRateMult();
  ...
  if (getRandom(100, shimmer) >= failRate) {
    ...
    if (Game.Has(drop) || Game.HasUnlocked(drop)) {
      ...
      if (getRandom(104, shimmer) < 0.1) drop = chooseBetter(Game.rareEggDrops, 104, shimmer);
      else drop = chooseBetter(Game.eggDrops, 105, shimmer);
    }
